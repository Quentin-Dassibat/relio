...
print("Create material for accuflux")
start = datetime.datetime.now()
dst = f"{hydrographicDirectory}/DEMx_SRTM30_wgs84_material.tif"
mask = DEM
relio.create_material(mask,dst,EPSG,1)
print("Elapsed time: ", datetime.datetime.now()-start)
del mask, dst

print("Convert material .tif file to PCRaster")
start = datetime.datetime.now()
src = f"{hydrographicDirectory}/DEMx_SRTM30_wgs84_material.tif"
dst = f"{hydrographicDirectory}/DEMx_SRTM30_wgs84_material_pcraster.map"
mask = DEM
...
mater = f"{hydrographicDirectory}/DEMx_SRTM30_wgs84_material_pcraster.map"
...
print("Add indexing to the filtered accuflux on a second band to further retrieve manually inflowing pixels")
src = f"{hydrographicDirectory}/DEMx_SRTM30_wgs84_pcraster_accuflux_sup{str(epsilonAccuflux)}.tif"
dst = f"{hydrographicDirectory}/DEMx_SRTM30_wgs84_pcraster_accuflux_sup{str(epsilonAccuflux)}_indexing.tif"
relio.add_pixelIndexing(src,dst,EPSG)


print("Total Elapsed Time: ", datetime.datetime.now()-globstart)

with open(f"{tmpDirectory}/log.txt", 'a') as file:
    file.write(f"INITIALIZEaccuflux.py Elapsed Time: {str(datetime.datetime.now()-globstart)}\n")